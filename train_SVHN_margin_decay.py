...
def train_kmargin(epoch):
    ...
    net.train()
    batch_index = 0
    # epoch starts at 1 -> 200
    x = (epoch/args.epochs)
    margin = 1 - ((x**args.margin_decay_order)*(1-args.margin))

    for batch_index, (images, labels) in enumerate(kmargin_accumulate(train_loader, net, args.batch_size, args.gpu, args.k_warm, margin)):
        if args.gpu:
            labels = labels.cuda()
            images = images.cuda()
...
if __name__ == '__main__':
    ...
    args_suffix = f"_O{args.optimizer}_B{args.batch_size}_LR{args.lr}" + f"_K{args.k_warm}_M{args.margin}_MDO{args.margin_decay_order}" if args.kmargin else ""
    optim_from_args = {
    ...
    writer = SummaryWriter(log_dir=os.path.join(
        settings.LOG_DIR, 'SVHN', args.net, 'kmargin_linear' if args.kmargin else 'standard',
    ...
    acc_train = eval_training(train_unaugmented_loader, 0, tag='Train')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 5 == 0:
            acc_train = eval_training(train_unaugmented_loader, epoch, tag='Train')
